...
export class TransacoesComponent implements OnInit {
  ...
  inicializarForm(): void {
    ...
    // Quando o tipo muda, ajustar validações
    this.form.get('tpTransacao')?.valueChanges.subscribe((tipo: TipoTransacao) => {
      ...
      if (tipo === 'Transferencia') {
        ...
        cdCategoria?.clearValidators();
        cdCategoria?.setValue(null);
      } else {
        cdContaDestino?.clearValidators();
        cdContaDestino?.setValue(null);
  ...
  carregarContas(): void {
    this.contaService.obterContas().subscribe({
      next: (res: any) => {
        ...
        this.contas = (lista as any[]).map(c => ({
          label: c.nmConta,
          value: c.cdConta,
        }));
      },
  ...
  carregarCategorias(): void {
    this.categoriaService.obterCategorias().subscribe({
      next: (res: any) => {
        const lista = res.dados ?? [];
        this.categorias = (lista as any[]).map(c => ({
          label: c.nmCategoria,
          value: c.cdCategoria,
        }));
      },
  ...
  get tipoSelecionado(): TipoTransacao | null {
    return this.form.get('tpTransacao')?.value;
  }

  get isTransferencia(): boolean {
    return this.tipoSelecionado === 'Transferencia';
  }

  abrirDialogNovo(): void {
    this.modoEdicao = false;
    this.cdTransacaoSelecionada = null;
    this.form.reset();
  ...
  abrirDialogEdicao(transacao: Transacao): void {
    ...
    this.form.patchValue({
      ...transacao,
      dtTransacao: transacao.dtTransacao ? new Date(transacao.dtTransacao) : null,
    });
    this.dialogVisivel = true;
  }
  ...
  salvar(): void {
    ...
    const dados: Transacao = {
      ...this.form.value,
      dtTransacao: this.form.value.dtTransacao instanceof Date
        ? this.form.value.dtTransacao.toISOString()
        : this.form.value.dtTransacao,
    };
  ...
  getTipoSeverity(tipo: TipoTransacao): 'success' | 'danger' | 'info' {
    switch (tipo) {
      ...
      case 'Despesa': return 'danger';
      case 'Transferencia': return 'info';
    }
  }

  getTipoLabel(tipo: TipoTransacao): string {
    return tipo === 'Transferencia' ? 'Transferência' : tipo;
  }
  ...
  getTotalReceitas(): number {
    return this.transacoes
      .filter(t => t.tpTransacao === 'Receita')
      .reduce((sum, t) => sum + t.vlTransacao, 0);
  }

  getTotalDespesas(): number {
    return this.transacoes
      .filter(t => t.tpTransacao === 'Despesa')
      .reduce((sum, t) => sum + t.vlTransacao, 0);
  }
